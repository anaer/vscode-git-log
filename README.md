...
- Branch cleanup from the Branches heading: one dialog lists local branches whose upstream is gone and those already merged into the current branch, showing the unmerged commit count and last commit date for each. Gone branches are ticked by default, unmerged ones never are. The batch runs one deletion per branch and reports the branches that failed instead of aborting the rest.
- Right-clicking a slash-delimited folder node under Local, Remote, or Tags opens a preview listing every branch beneath it. Nothing is preselected and the delete button stays disabled until you tick the items to remove; confirming runs one cross-kind batch delete — local branches via `git branch -d`, remote branches via `git push --delete`, tags via `git tag -d` — with per-item failures reported instead of aborting the rest.
...
- Checkout, Checkout Revision, Branch, Tag, Fetch, Pull, Push, Publish Branch, Cherry-pick, Revert, Merge, Rebase, Reset, and branch rename/delete operations, with context menus for commits and Local, Remote, Tag, and HEAD refs. The Push entry becomes Publish Branch while the current branch has no upstream, so it pushes and records the upstream in one action.
- The HEAD context menu offers **Create Orphan Branch…**, which starts a branch with no parent commit via `git switch --orphan`. It removes all tracked files from the working tree while keeping untracked and ignored files, requires a confirmation, and the new branch appears in the ref tree only after its first commit.
- Shallow clones show a "history is truncated" notice above the commit list with a **Fetch full history** action that runs `git fetch --unshallow`. For a single-branch clone whose fetch refspec covers only one branch, the confirmation lists the change verbatim and the refspec is widened to all branches, so previously hidden remote branches appear after the fetch.
...
- Local Branch context menus provide a confirmed `Delete…` action for non-current branches that defaults to force deletion. Normal deletion failures for unmerged branches also offer a warning-colored Delete recovery action, and error notices remain visible for five seconds before closing automatically.
